...
namespace IndicoV2.Converters.CastingConverter
{
    public class CastingConverter<TCastFrom> : JsonConverter
    {
        ...
        {
            var castOperator = targetTyped.GetMethod("op_Implicit", new[] {typeof(TCastFrom)}) ?? throw new InvalidOperationException($"Cannot cast form ${typeof(TCastFrom).Name}, operator not found.");
